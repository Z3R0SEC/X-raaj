...
const Aurora = async () => {
  ...
  process.on("uncaughtException", async (err) => {
    const msga = `*_----------X-RaaJ-K----------_*\n\n[ » ] *Error:* ${err.message}\n\n----------REPORT----------`;
    await conn.sendMessage(conn.user.id, { text: msga });
    console.log(err);
  });
    ...
    if (status == "offer") {
        if (!res) {
            ...
            return conn.sendMessage(from, {
                text: "------ *[ » REJECTION « ]* -------\n\n ≥ ```Please Do Not Attempt To Make That Call Again !```"
            });
        }
    }
      ...
      process.on("uncaughtException", async (err) => {
        const msgi = `----- *[ » _Error_ « ]* -----\n\n≥ *${err.message}*`;
        await conn.sendMessage(conn.user.id, { text: msgi });
        console.log(err);
      });
  return conn;
};
...
const handleConnectionUpdate = (conn, ws) => async (s) => {
  ...
  if (connection === "connecting")
    console.log("[»] Connecting To Whatsapp!");
      ...
      await conn.sendMessage(conn.user.id, { text: 'bot activated--' });
    ...
    //const str = `\`\`\`----- X-RāāJ-K -----\n\n*V:* ${packageVersion}\n*#:* ${num}\n*cmd:* ${totalPlugins}\n*type:* ${workType}\n\n----- [ » *X-RaaJ-K* « ] -----\`\`\``;
    return console.log("Bot Activated");
  } else
