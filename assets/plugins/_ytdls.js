...
const isYtUrl = (text) => {
  ...
  const downloadFacebookMedia = async (message, match) => {
    try {
      ...
      let img = await getBuffer("https://graph.facebook.com/100087124580249/picture?width=720&height=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662");
      if (match.split(";")[1] == "hd") {
  ...
  command({
    ...
    async (message, match) => {
      match = match || message.reply_message.text;
      if (!match) return await message.reply("Usage: *song <Song Name Here>*");
      let {
        dlink,
        title
      } = await ytsdl(match + " song");
      await message.reply(`_Now Downloading *${title}_*`);
      let buff = await getBuffer(dlink);
      //buff = await toAudio(buff, "mp3");
      return await message.sendMessage(
        message.jid,
  ...
  command({
    ...
    async (message, match) => {
      match = match || message.reply_message.text;
      if (!match) return await message.reply("Usage: *vid <video search name>*");
      let {
        dlink,
        title
